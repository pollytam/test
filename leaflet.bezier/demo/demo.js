d3.select("#get_data_button").on("click", get_data);

function get_data(){
    // d3.event.preventDefault();
    var year = d3.select("#year").node().value
    var quarter = d3.select("#quarter").node().value
    var origin = d3.select("#origin").node().value

    console.log(year);
}

$(function () {

    var map = L.map('map').setView([40.9270786, -79.861243], 3);

    L.tileLayer('https://cartodb-basemaps-c.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png', {
        maxZoom: 50,
        id: 'mapbox.streets'
    }).addTo(map);


    var dash_straight = {
        color: 'rgb(145, 146, 150)',
        fillColor: 'rgb(145, 146, 150)',
        dashArray: 8,
        opacity: 0.8,
        weight: '1',
    };

    var getbydate = document.getElementById("getbydate")
    var getbydate = getbydate.options[getbydate.selectedIndex].value

    var form = document.getElementById("form")
    form.addEventListener("submit", function(e){
        e.preventDefault()
        
        fetch("http://localhost:5000/getbydate/<year>/<quar>/<origin>", {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'},
            // body: form.elements
            data: getbydate
        }).then( 
            data=>{
                // console.log(data)
                return data.json()
            } 
        ).then(
            res => {
                console.log("WE ARE getting data back")
                console.log(res)

            }
        )
    })

    L.bezier({
        path: [
            [
                {lat: 37.62, lng: -122.375000, slide: 'LEFT_ROUND'},//Sri Lanka
                // {lat: -25.2744, lng: 133.7751, slide: 'LEFT_ROUND'},//Australia
                {lat: 40.67, lng: -73.940002}//Japan
            ],
            // [
            //     {lat: 7.8731, lng: 80.7718, slide: 'RIGHT_ROUND'},//Sri Lanka
            //     {lat: 3.1390, lng: 101.6869}
            // ],
            // [
            //     {lat: 7.8731, lng: 80.7718, slide: 'RIGHT_ROUND',deep:"8"},//Sri Lanka
            //     {lat: 41.8719, lng: 12.5674}
            // ],[
            //     {lat: -25.2744, lng: 133.7751},//Australia
            //     {lat: -40.9006, lng: 174.8860}//Japan
            // ],
            // [
            //     {lat: 7.8731, lng: 80.7718, slide: 'RIGHT_ROUND'},
            //     {lat: -18.7669, lng: 46.8691},
            // ]
        ],

        icon: {
            path: "plane.png",
            size: '1px'
        }
    }, dash_straight).addTo(map);


});